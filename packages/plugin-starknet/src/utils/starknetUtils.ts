...
export const getStarknetProvider = (runtime: AgentRuntime) => {
  return new RpcProvider({
    nodeUrl: runtime.getSetting("STARKNET_RPC_URL"),
  });
};

export const getStarknetAccount = (runtime: AgentRuntime) => {
  return new Account(
...
export const parseFormattedAmount = (amount: string) =>
  amount.replace(/,/g, "");
...
export async function fetchWithRetry<T>(
  ...
  for (let retryCount = 0; retryCount <= maxRetries; retryCount++) {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Coingecko API HTTP status: ${response.status}`);
      }
