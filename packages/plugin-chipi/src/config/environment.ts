...
export const chipiEnvSchema = z.object({
    CHIPI_PUBLIC_API_KEY: z.string().min(1, "Chipi key is required"),
});

export type ChipiConfig = z.infer<typeof chipiEnvSchema>;

export async function validateChipiConfig(
    ...
    try {
        const config = {
            ...
            CHIPI_SECRET_KEY:
                runtime.getSetting("CHIPI_SECRET_KEY") || 
        ...
        if (error instanceof z.ZodError) {
            const errorMessages = error.errors
                ...
                .join("\n");
            throw new Error(
                `Chipi configuration validation failed:\n${errorMessages}`
            );
        }
        throw error;
    }
